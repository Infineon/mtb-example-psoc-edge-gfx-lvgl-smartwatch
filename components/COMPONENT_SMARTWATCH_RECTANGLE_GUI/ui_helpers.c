...
{
    if(id == _UI_BAR_PROPERTY_VALUE_WITH_ANIM) lv_bar_set_value(target, val, (lv_anim_enable_t)LV_ANIM_ON);
    if(id == _UI_BAR_PROPERTY_VALUE) lv_bar_set_value(target, val, (lv_anim_enable_t)LV_ANIM_OFF);
}
...
{
    if(id == _UI_SLIDER_PROPERTY_VALUE_WITH_ANIM) lv_slider_set_value(target, val, (lv_anim_enable_t)LV_ANIM_ON);
    if(id == _UI_SLIDER_PROPERTY_VALUE) lv_slider_set_value(target, val, (lv_anim_enable_t)LV_ANIM_OFF);
}
...
void _ui_arc_increment(lv_obj_t * target, int val)
{
    ...
    lv_obj_send_event(target, (lv_event_code_t)LV_EVENT_VALUE_CHANGED, 0);
}
...
void _ui_flag_modify(lv_obj_t * target, int32_t flag, int value)
{
    if(value == _UI_MODIFY_FLAG_TOGGLE) {
        if(lv_obj_has_flag(target, (lv_obj_flag_t)flag)) lv_obj_remove_flag(target, (lv_obj_flag_t)flag);
    ...
    else lv_obj_remove_flag(target, (lv_obj_flag_t)flag);
}
