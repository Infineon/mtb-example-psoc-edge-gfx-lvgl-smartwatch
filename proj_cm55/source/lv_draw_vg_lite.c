...
/* Preference score assigned so this draw unit takes priority over others. */
#define VG_LITE_DRAW_UNIT_PREFERENCE_SCORE 80

extern volatile bool gpu_enable;
...
static LV_ATTRIBUTE_FAST_MEM bool check_image_is_supported(const lv_draw_image_dsc_t * dsc)
{
    return lv_vg_lite_is_src_cf_supported((lv_color_format_t)dsc->header.cf);
}

static LV_ATTRIBUTE_FAST_MEM bool check_arc_is_supported(const lv_draw_arc_dsc_t * dsc)
{
    if(dsc->img_src == NULL) {
        return true;
    }
...
static LV_ATTRIBUTE_FAST_MEM void draw_execute(lv_draw_vg_lite_unit_t * u)
{
...
static LV_ATTRIBUTE_FAST_MEM int32_t draw_dispatch(lv_draw_unit_t * draw_unit, lv_layer_t * layer)
{
...
static LV_ATTRIBUTE_FAST_MEM int32_t draw_evaluate(lv_draw_unit_t * draw_unit, lv_draw_task_t * task)
{
    LV_UNUSED(draw_unit);

    /* Do not accept tasks when the GPU is disabled (LP/ULP mode) */
    if (!gpu_enable) {
        return -1;
    }
    ...
    switch(task->type) {
        /* NOTE: LV_DRAW_TASK_TYPE_LETTER and LV_DRAW_TASK_TYPE_LABEL
         * removed from VG-Lite acceleration per Infineon patch -
         * Infineon's VG-Lite implementation does not reliably support
         * bitmap font rendering via GPU. Labels fall back to SW draw. */
        case LV_DRAW_TASK_TYPE_FILL:
        case LV_DRAW_TASK_TYPE_BORDER:
    ...
    if(task->preference_score > VG_LITE_DRAW_UNIT_PREFERENCE_SCORE) {
        /* The draw unit is able to draw this task. */
        task->preference_score = VG_LITE_DRAW_UNIT_PREFERENCE_SCORE;
...
static LV_ATTRIBUTE_FAST_MEM int32_t draw_delete(lv_draw_unit_t * draw_unit)
{
...
static LV_ATTRIBUTE_FAST_MEM void draw_event_cb(lv_event_t * e)
{
