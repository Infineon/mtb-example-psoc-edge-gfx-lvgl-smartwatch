...
/* The CO5300 round AMOLED is shared by two UIs that have different pixel-format
 * needs, so the depth is selected by the active demo (see common.mk):
 *
 *   Benchmark demo (LV_USE_DEMO_BENCHMARK=1): 32-bit XRGB8888 frame buffer.
 *       The 4-byte element gives a width*4 row stride and full 8-bit/channel
 *       blending, which renders the benchmark's anti-aliased widget fonts
 *       cleanly. (Requires the expanded LV_DRAW_SW_SUPPORT_* set below.)
 *
 *   Smartwatch UI (LV_USE_DEMO_BENCHMARK=0): 16-bit RGB565 frame buffer.
 *       The SquareLine Studio export (components/COMPONENT_SMARTWATCH_ROUND_GUI)
 *       stores every image asset as RGB565 and ui.c hard-asserts
 *       LV_COLOR_DEPTH==16, so this UI must stay 16-bit.
 */
...
#define LV_DEF_REFR_PERIOD  (16U)      /**< [ms]*/
...
#else
    #define LV_DRAW_LAYER_SIMPLE_BUF_SIZE    (100 * 1024)   /**< [bytes]*/
#endif
...
#define LV_USE_DRAW_SW 1
    ...
    /* 32-bit XRGB8888 frame buffer (benchmark demo). The SW renderer must
     * support the 32-bit destination plus every glyph/image mask format it
     * blends into it (A8/AL88/L8/RGB565A8/RGB888), otherwise anti-aliased text
     * (A8 masks) silently disappears - the GPU draws shapes but glyph blits into
     * an unsupported destination become no-ops. Matches the proven reference. */
...
#else
    /* 16-bit RGB565 frame buffer (SquareLine Smartwatch UI). Glyph A8 masks
     * blend directly into the RGB565 destination, so the minimal set is enough;
     * the extra 32-bit/mask formats are disabled to save ITCM/code size. */
    ...
    /* IAR assembler cannot process GCC-style .S files used by helium blend */
    #ifdef __ICCARM__
...
#define LV_USE_DRAW_VG_LITE 1
    ...
    #define LV_USE_VG_LITE_DRIVER  0
        ...
        #define LV_VG_LITE_HAL_GPU_REVISION 0x40A
 ...
 *  @endcode
 */
#define LV_FONT_CUSTOM_DECLARE LV_FONT_DECLARE(ui_font_H1)\
...
/*Arc label widget - text along an arc path. Disabled to save ITCM.*/
